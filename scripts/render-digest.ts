...
(async () => {
  ...
  for (const sec of ['already_in_play', 'new_on_the_radar', 'worth_a_conversation'] as const) {
  ...
  for (const sec of ['worth_a_conversation', 'new_on_the_radar', 'already_in_play'] as const) {
    for (const p of plan.sections[sec]) {
      const r = renderRows.get(p.itemId);
  ...
  for (const p of [...plan.sections.worth_a_conversation, ...plan.sections.already_in_play]) {
    const r = renderRows.get(p.itemId);
    if (!r) continue;
  ...
  if (save) {
    const featured = (['worth_a_conversation', 'new_on_the_radar', 'already_in_play'] as const)
