...
(async () => {
  ...
  for (const sec of ['account_activity', 'new_on_the_radar', 'worth_a_conversation'] as const) {
  ...
  for (const sec of ['worth_a_conversation', 'new_on_the_radar', 'account_activity'] as const) {
    for (const p of plan.sections[sec]) {
      const r = renderRows.get(p.itemId);
  ...
  if (save) {
    const featured = (['worth_a_conversation', 'new_on_the_radar', 'account_activity'] as const)
